...
所有文档统一放这里。`docs\` 已重新规划目录结构，所有 AI Agent 创建或移动文档时必须遵循以下布局：

```
E:\sEMG_B_Project\docs\
├── 现用源文档（根级，每个主题只留一份最新，无版本号后缀）
│   ├── 研究日志.docx / 研究日志.md
│   ├── 硬件说明.docx / 硬件说明.md
│   ├── 肌电算法说明.docx / 肌电算法说明.md
│   ├── 软件设计.docx / 软件设计.md
│   └── 论文.docx
├── <主题>备份/            ← 历史版本按时间戳存入此处（根级，不再放资料/）
│   ├── 论文备份/  研究日志备份/  硬件说明备份/
│   └── 肌电算法说明备份/  软件设计备份/
├── 其他文档/             ← 早期/辅助文档（小程序设计、系统架构、系统设计、文档索引、云开发环境指南、固件与小程序架构）
└── 资料/                 ← 仅参考资料，不再含任何备份
    ├── 参考文献/          ← PDF + 参考文献清单/高中生课题引用包/作者索取邮件模板
    ├── 格式模板/          ← 研究日志/论文/答辩/PPT 规范与模板
    ├── 演示文稿/          ← 小程序操作指南.pptx 等
    ├── 芯片手册/          ← Arduino UNO R4 / ESP32-S3 / 干电极传感器资料
    └── 论文与研究日志示例/ ← 外部优秀范例 PDF
```

### 备份与更新流程（必须按顺序）

1. 修改任何 `docs\` 下源文档**之前**，先按时间戳备份旧版到对应的根级 `<主题>备份/` 文件夹（如 `docs\论文备份\论文备份_YYYYMMDD_HHMMSS.docx`），根目录始终只留一份最新版
2. 写入新版到 `docs\`（无版本号后缀，覆盖原文件）
3. `docs\` 根目录下只保留当前最新源文档；历史版本一律在 `<主题>备份/` 内，参考资料只在 `资料/` 的 5 个子目录里

### 命名规则

- 当前版：`文档名.docx` / `文档名.md`（无版本号/时间戳后缀，位于 `docs\` 根）
- 备份版：`文档名_YYYYMMDD_HHMMSS.docx`（时间戳，存入根级 `<主题>备份\`）
- 参考资料（`资料/` 下）保持原文件名，不附加版本号
