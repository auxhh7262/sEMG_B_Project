...
- **字体**（tech 与 paper 一致，run 级强制覆盖 pandoc 默认 TNR）：
  - 论文题目（首个 H1）：黑体 小三(15pt) 加粗 居中
  - 一级标题（后续 H1，如"一、引言"）：黑体 小四(12pt) 加粗
  - 正文：宋体(eastAsia) / Times New Roman(ascii) 小四(12pt)，1.5 倍行距
  - 摘要标题：黑体 小四(12pt) 加粗（自动拆分"摘要：xxx"→独立"摘要"标题 + 正文）
  - 摘要正文：宋体 五号(10.5pt)
  - 关键词：宋体 小四(12pt)
  - 参考文献 [n] 条目：Times New Roman / 宋体 五号(10.5pt)
  - 图名：宋体 五号(10.5pt) 居中
  - ⚠️ pandoc 在 run 级直接写入 rFonts 覆盖样式层，故 `style_paper_blocks()` 逐段逐 run 强制设字体 + 行距，仅靠 `setup_paper_fonts()` 设 Normal 样式不够
- **表格**（⚠️ 两者唯一区别）：
