...
try:
    ...
    print("ERROR: threading module not available")
    input("Press Enter to exit...")
    sys.exit(1)
...
try:
    ...
    print("ERROR: tkinter not available. Please use Python 3.x with tkinter support")
    input("Press Enter to exit...")
    sys.exit(1)
...
def kill_previous():
    ...
    try:
        ...
        print(f"Warning: Failed to kill previous processes: {e}")
        ...
        try:
            if ser is None:
                try:
                    ...
                    append(f'Port {PORT} opened, waiting for device...', 'BOOT')
                    # Wait for boot data
...
def upload_workflow():
    ...
    try:
        ...
        log_msg(f'FIRMWARE_DIR={FIRMWARE_DIR}')
        time.sleep(0.5)
